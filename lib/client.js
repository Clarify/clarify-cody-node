...
class Client {
  constructor (apiToken, opts) {
    if (!apiToken) throw new Error('Clarify API Token required');
  ...
  adminConversationsUsage (startDate, endDate, callback) {
    this.get('admin/conversations/usage', {start: startDate, end: endDate}, callback);
  }

  adminConversationsUsageAggregate (daysAgo, callback) {
    this.post('admin/conversations/usage/aggregate', {days_ago: daysAgo}, callback);
  }
};
